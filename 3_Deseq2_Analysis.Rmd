...
PD_less_mouse <- subset(Pd, subset = Mouse != "4788")
...
pseudo_pd <- AggregateExpression(PD_less_mouse, assays = "RNA", return.seurat = T, group.by = c("Genotype", "Mouse"))

pseudo_pd$Genotype <- paste(pseudo_pd$Genotype)
Idents(pseudo_pd) <- "Genotype"

print(tail(Cells(pseudo_pd)))
...
B <- pseudo_pd_celltype[["RNA"]]$counts
...
for (j in B@Dimnames[[2]]){
  ...
  mouse <- paste(strsplit(j,split = "_")[[1]][2],strsplit(j,split = "_")[[1]][3],sep="_")
  
  #pull the associated normalization factor
  norm_factor <- num_counts_norm_factor[mouse]
...
#note, entrys need to be integers, so all scaled values are rounded to the nearest whole number
B <- round(t(t(B)*num_counts_norm_factor_celltype))


#set the pseudo_pd_counts
pseudo_pd_cell_num_norm <- pseudo_pd_celltype
pseudo_pd_cell_num_norm <- SetAssayData(object = pseudo_pd_cell_num_norm, layer = "counts", new.data = B)

#prepare Deseq2
pseudo_pd_cell_num_norm$sctype_classification.Genotype <- paste(pseudo_pd_cell_num_norm$sctype_classification,pseudo_pd_cell_num_norm$Genotype, sep = "_" )

Idents(pseudo_pd_cell_num_norm) <- "sctype_classification.Genotype"
...
for (Cell_type in unique(pseudo_pd_cell_num_norm$sctype_classification)){
  ...
  print(EnhancedVolcano(pseudo_pd_cell_num_norm.de,
    ...
    max.overlaps = 100,
    colCustom = keyvals,
    title = paste(Cell_type,"Volcano Plot",sep = " ")
    ))
...
VlnPlot(Pd,assay = "RNA",layer = 'scale.data',
        ...
        idents=c(cta,ctwt),
        add.noise=FALSE,
        pt.size=0.75) + ggtitle(paste(feature,"Expression in",Cell_type,sep=" ")) +ggplot2::scale_fill_manual(values = c("#00bfc4","#f8766d"))+ ylab("Log Normalized Gene Expression")
...
pseudo_pd.normed.de <- cbind("Gene"=rownames(pseudo_pd.normed.de), pseudo_pd.normed.de)
  ...
  print(EnhancedVolcano(pseudo_pd.normed.de,
    ...
    max.overlaps = 100,
    colCustom = keyvals,
    title = paste("All Cells","Volcano Plot",sep = " ")
    ))
  #save the plot
